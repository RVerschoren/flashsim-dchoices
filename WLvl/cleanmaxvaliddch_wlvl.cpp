 ...
 *
 * Class implementing a greedier version of the Ban wear leveling scheme, which cleans the block with the highest number
...
WLvlImpl_CleanMaxValidDCh::WLvlImpl_CleanMaxValidDCh(FtlParent* FTL, const double p = WLVL_BAN_P, const uint d = DCHOICES_D)
  : Wear_leveler(ftl), p(p), d(d)
{
}
...
enum status
WLvlImpl_CleanMaxValidDCh::suggest_WF(Address& WFSuggestion, const std::vector<Block*>& doNotPick)
{
    if (RandNrGen::get() < p) { // Pick a random block
        ...
        greedy_block<uint>(WFSuggestion, costFunc, ignorePred);
        return SUCCESS;
    }
    return FAILURE;
}
