...
//
//    const double dnumLPN = (1.0 - SPARE_FACTOR) * BLOCK_SIZE * PLANE_SIZE *
//                           DIE_SIZE * PACKAGE_SIZE * SSD_SIZE;
//    const ulong numLPN = static_cast<ulong>(std::floor(dnumLPN));
//    const ulong maxHotLPN =
//      static_cast<ulong>(std::floor(HOT_FRACTION * dnumLPN));
...
main(int argc, char* argv[])
{
    ...
    /// Override with command line settings
    const ssd::SSD_Run_Params params = setup_synth(argc, argv, "ban", WL_BAN_PROB);
