...
typedef std::map<LPN, LPN> LPNMap;//Maps old LPNs (first) to new LPNs (second in pair)

struct LPNNum{
    LPN lpn;
    uint num;
    LPNNum(const LPN &l, const uint numberOfAppearances) : lpn(l), num(numberOfAppearances){}
};

LPNMap getNewLPNs(const LPNCount &count){
