...
setup(int argc, char* argv[])
{
    ...
    sstr << ftlstr << "traceban";
    sstr << "-b" << BLOCK_SIZE;
    sstr << "-d" << DCHOICES_D;
    sstr << "-sf" << std::setprecision(2) << SPARE_FACTOR;
    if (FTL_IMPLEMENTATION == IMPL_DWF || FTL_IMPLEMENTATION == IMPL_HCWF || FTL_IMPLEMENTATION == IMPL_COLD) {
...
main(int argc, char* argv[])
{
    ...
    const std::string traceID = TRACEFILE.substr(0, 4);
    EventReader evtRdr(TRACEFILE, NUMREQUESTS, READER_MODE, create_oracle_filename(traceID, HOT_FRACTION, 1));
    ...
    PLANE_SIZE = static_cast<uint>(
      std::ceil(static_cast<double>(numLPN) / static_cast<double>(BLOCK_SIZE * (1.0 - SPARE_FACTOR))));
