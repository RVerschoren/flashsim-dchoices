 ...
 * "A SPACE-EFFICIENT FLASH TRANSLATION LAYER FOR COMPACTFLASH SYSTEMS by Kim
 * et. al."
 *
 * Notice: Startup procedures are not implemented as the drive is empty every
 * time
 ...
 * every time a page read is out a cache log page. A cache log page usually hold
 * approx.
 * 1000 mappings.
 *
...
/* Comparison class for use by FTL to sort the LogPageBlock compared to the
 * number of pages written. */
bool
LogPageBlock::operator()(const LogPageBlock& lhs, const LogPageBlock& rhs) const
{
...
FtlImpl_Bast::read(Event& event) {
	...
	{
		Address returnAddress = Address(logBlock->address.get_linear_address() +
		                                logBlock->pages[eventAddress.page],
		                                PAGE);
		event.set_address(returnAddress);
	} else if ((data_list[lookupBlock] == -1 && logBlock != NULL &&
	            logBlock->pages[eventAddress.page] == -1) ||
	           (data_list[lookupBlock] == -1 && logBlock == NULL))
	{
...
enum status
FtlImpl_Bast::write(Event& event) {
	LogPageBlock* logBlock = NULL;
	...
	if (numValid < BLOCK_SIZE)
	{
		if (logBlock->pages[eventAddress.page] != -1) {
			...
			Address(logBlock->address.get_linear_address() +
			        logBlock->pages[eventAddress.page],
			        PAGE);
	...
	if (data_list[lba] != -1)
	{
		...
		if (controller.get_block(replace)->get_state(offset) != EMPTY)
...
FtlImpl_Bast::trim(Event& event) {
	...
	if (logBlock != NULL &&
	        logBlock->pages[eventAddress.page] != -1) // If page is in the log block
	{
		returnAddress = Address(logBlock->address.get_linear_address() +
		                        logBlock->pages[eventAddress.page],
		                        PAGE);
		...
		if (lBlock->get_state() ==
			...
			Block_manager::instance()->erase_and_invalidate(event,
			        returnAddress, LOG);
		}
	}
	...
	if (data_list[lookupBlock] != -1) // Datablock
	{
		...
		if (dBlock->get_state() ==
			...
			Block_manager::instance()->erase_and_invalidate(event, dataAddress,
			        DATA);
		}
	}
...
void
FtlImpl_Bast::allocate_new_logblock(LogPageBlock* logBlock, long lba,
                                    Event& event)
{
...
void
FtlImpl_Bast::dispose_logblock(LogPageBlock* logBlock, long lba)
{
	log_map.erase(lba);
	delete logBlock;
}
...
bool
FtlImpl_Bast::is_sequential(LogPageBlock* logBlock, long lba, Event& event)
{
...
bool
FtlImpl_Bast::random_merge(LogPageBlock* logBlock, long lba, Event& event)
{
	...
	for (uint i = 0; i < BLOCK_SIZE; i++) {
		...
		if (logBlock->pages[i] != -1)
			readAddress.set_linear_address(
			    logBlock->address.get_linear_address() + logBlock->pages[i],
			    PAGE);
		...
		if (controller.get_state(readAddress) ==
		        INVALID) // A page might be invalidated by trim
			continue;
	...
	Block_manager::instance()->erase_and_invalidate(event, logBlock->address,
	        LOG);
...
void
FtlImpl_Bast::update_map_block(Event& event)
{
	Event writeEvent =
...
void
FtlImpl_Bast::print_ftl_statistics()
{
