...
enum status
FtlImpl_BDftl::read(Event& event) {
...
enum status
FtlImpl_BDftl::write(Event& event) {
	uint dlpn = event.get_logical_address();
	uint dlbn = dlpn / BLOCK_SIZE;
	bool handled = false;
	...
	if (block_map[dlbn].optimal)
	{
		...
		if (block_map[dlbn].pbn == -1u && dlpn % BLOCK_SIZE == 0)
			block_map[dlbn].pbn = Block_manager::instance()
			->get_free_block(DATA, event)
			.get_linear_address();
		...
		if (block_map[dlbn].pbn != -1u) {
			...
			if (block_map[dlbn].nextPage == dppn) {
				...
				Block* b = controller.get_block(Address(startAdr, PAGE));
				...
				for (uint i = 0; i < numPages; i++) {
					...
					if (current.ppn != -1) {
						update_translation_map(current,
						                       block_map[dlbn].pbn + i);
						current.create_ts = event.get_start_time();
						current.modified_ts = event.get_start_time();
						current.cached = true;
						trans_map.replace(trans_map.begin() + startAdr + i,
						                  current);
				...
				Block* block =
				    controller.get_block(Address(block_map[dlbn].pbn, BLOCK));
	...
	controller.stats.numMemoryRead +=
	    3; // Block-level lookup + range check + optimal check
...
long
FtlImpl_BDftl::get_free_biftl_page(Event& event)
{
	// Important order. As get_free_data_page might change current.
	long free_page = -1;
...
enum status
FtlImpl_BDftl::trim(Event& event) {
	uint dlpn = event.get_logical_address();
	uint dlbn = dlpn / BLOCK_SIZE;
	...
	} else   // DFTL lookup
	{
		...
		// Update trim map and update block map if all pages are trimmed. i.e.
		// the state are reseted to optimal.
...
void
FtlImpl_BDftl::cleanup_block(Event& event, Block* block)
{
	...
	for (uint i = 0; i < BLOCK_SIZE; i++) {
		...
		// When valid, two events are create, one for read and one for write.
		// They are chained and the controller are
		// called to execute them. The execution time is then added to the real
		// event.
		if (block->get_state(i) == VALID) {
			// Set up events.
			Event readEvent = Event(READ, event.get_logical_address(), 1,
			                        event.get_start_time());
			readEvent.set_address(
			    Address(block->get_physical_address() + i, PAGE));
			...
			Address dataBlockAddress =
			    Address(get_free_data_page(event, false), PAGE);
			writeEvent.set_address(dataBlockAddress);
			writeEvent.set_replace_address(
			    Address(block->get_physical_address() + i, PAGE));
			...
			// reverse_trans_map[block->get_physical_address()+i],
			// block->get_physical_address()+i, dataPpn);
			invalidated_translation
			[reverse_trans_map[block->get_physical_address() + i]] = dataPpn;
			...
			controller.stats.numMemoryWrite =
			    +3; // GTD Update (2) + translation invalidate (1)
		}
	}
	...
	for (std::map<long, long>::const_iterator i =
	            invalidated_translation.begin();
	        i != invalidated_translation.end(); ++i) {
		long real_vpn = (*i).first;
		long newppn = (*i).second;

		// Update translation map ( it also updates the CMT, as it is stored
		// inside the GDT )
...
bool
FtlImpl_BDftl::block_next_new()
{
	return (currentDataPage == -1 ||
	        currentDataPage % BLOCK_SIZE == BLOCK_SIZE - 1);
}

void
FtlImpl_BDftl::print_ftl_statistics()
{
	printf("FTL Stats:\n");
