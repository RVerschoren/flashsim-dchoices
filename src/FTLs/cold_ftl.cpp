...
void
FtlImpl_COLD::initialize(const ulong numLPN)
{
...
FtlImpl_COLD::FtlImpl_COLD(Controller& controller, HotColdID* hcID,
                           const uint d)
...
void
FtlImpl_COLD::gca_collect_COLD(const Event& event, Address& victimAddress,
                               ...
                               const std::vector<Address>& doNotPick)
{
    if (replacingCWF) // If replacing CWF, pick a cold block (limits hot->cold conversions)
	{
        ...
        [this](const Address& addr) {
            return get_pages_valid(addr)
                    + (get_block_hotness(addr)? (BLOCK_SIZE+1) : 0);
        };
...
enum status
FtlImpl_COLD::write(Event& event) {
#ifdef DEBUG
...
//std::cout << "BEGIN LOOP " << std::endl << std::endl;
    ...
    {
        const bool HWFInitiated = get_next_page(HWF) != SUCCESS;
        assert(not HWFInitiated or get_block(HWF)->get_pages_empty() == 0);
        assert(HWFInitiated or get_block(CWF)->get_pages_empty() == 0);
...
void
FtlImpl_COLD::modifyFTL(const ulong lpn, const Address& address)
{
    map[lpn] = address;
}

void
FtlImpl_COLD::check_valid_pages(const ulong numLPN)
{
    uint numPages = 0;
    for (uint package = 0; package < SSD_SIZE; package++) {
        for (uint die = 0; die < PACKAGE_SIZE; die++) {
...
void
FtlImpl_COLD::check_block_hotness()
{
    uint numBlocks = 0;
    for (uint package = 0; package < SSD_SIZE; package++) {
        for (uint die = 0; die < PACKAGE_SIZE; die++) {
...
void
FtlImpl_COLD::check_ftl_hotness_integrity()
{
