 ...
 * "DFTL: A Flasg Translation Layer Employing Demand-based Selective Caching og
...
FtlImpl_DftlParent::mpage_last_visited_time_compare(
    const FtlImpl_DftlParent::MPage& mpage)
{
	if (!mpage.cached)
	...
	: FtlParent(controller)
{
	...
	printf("Total required bits for representation: Address size: %i Total per "
	       "page: %i \n",
...
void
FtlImpl_DftlParent::consult_GTD(long /*dlpn*/, Event& event)
{
...
void
FtlImpl_DftlParent::reset_MPage(FtlImpl_DftlParent::MPage& mpage)
{
	mpage.create_ts = -2;
	mpage.modified_ts = -2;
...
bool
FtlImpl_DftlParent::lookup_CMT(long dlpn, Event& event)
{
...
long
FtlImpl_DftlParent::get_free_data_page(Event& event)
{
	return get_free_data_page(event, true);
}

long
FtlImpl_DftlParent::get_free_data_page(Event& event, bool insert_events)
{
	if (currentDataPage == -1 ||
	        (currentDataPage % BLOCK_SIZE == BLOCK_SIZE - 1 && insert_events))
		Block_manager::instance()->insert_events(event);

	if (currentDataPage == -1 || currentDataPage % BLOCK_SIZE == BLOCK_SIZE - 1)
		currentDataPage = Block_manager::instance()
...
void
FtlImpl_DftlParent::resolve_mapping(Event& event, bool isWrite)
{
...
void
FtlImpl_DftlParent::evict_page_from_cache(Event& event)
{
	while (cmt >= totalCMTentries) {
		// Find page to evict
		MpageByLastVisited::iterator evictit =
		    boost::multi_index::get<1>(trans_map).begin();
		MPage evictPage = *evictit;

		assert(evictPage.cached && evictPage.create_ts >= 0 &&
		       evictPage.modified_ts >= 0);
		...
		if (evictPage.create_ts != evictPage.modified_ts) {
			...
			Event write_event = Event(WRITE, event.get_logical_address(), 1,
			                          event.get_start_time());
			write_event.set_address(Address(0, PAGE));
			write_event.set_noop(true);

			if (controller.issue(write_event) == FAILURE) {
				assert(false);
			}
...
void
FtlImpl_DftlParent::evict_specific_page_from_cache(Event& event, long lba)
{
	// Find page to evict
...
void
FtlImpl_DftlParent::update_translation_map(FtlImpl_DftlParent::MPage& mpage,
        long ppn)
{
	mpage.ppn = ppn;
