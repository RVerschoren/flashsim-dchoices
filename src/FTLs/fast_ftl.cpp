	...
	: FtlParent(controller)
{
	...
	printf(
	    "Total required bits for representation: %i (Address: %i Block: %i) \n",
...
void
FtlImpl_Fast::initialize_log_pages()
{
	if (log_pages != NULL)
		return;
...
FtlImpl_Fast::read(Event& event) {
	...
	if (!found)
	{
		if (sequential_logicalblock_address == lookupBlock &&
		        sequential_offset > lbnOffset) {
			event.set_address(Address(
			                      sequential_address.get_linear_address() + lbnOffset, PAGE));
		} else if (data_list[lookupBlock] != -1) { // If page is in the data block
			event.set_address(
			    Address(data_list[lookupBlock] + lbnOffset, PAGE));
		} else { // Empty
...
enum status
FtlImpl_Fast::write(Event& event) {
	initialize_log_pages();
	...
	{
		Address newBlock =
		Block_manager::instance()->get_free_block(DATA, event);
	...
	} else
	{
		...
		Address dataAddress =
		Address(data_list[logicalBlockAddress] + lbnOffset, PAGE);
...
FtlImpl_Fast::trim(Event& event) {
		...
		for (int i = 0; i < currentBlock->numPages; i++) {
			if (currentBlock->aPages[i] == (long)event.get_logical_address()) {
				...
				if (block->get_state() ==
					...
					Block_manager::instance()->erase_and_invalidate(
					    event, currentBlock->address, LOG);
	...
	if (!found)
	{
		if (sequential_logicalblock_address == lookupBlock &&
			...
			if (block->get_state() ==
			        INACTIVE) { // All pages invalid, force an erase. PTRIM style.
				Block_manager::instance()->erase_and_invalidate(event, address,
				        LOG);
				sequential_logicalblock_address = -1;
			}
...
void
FtlImpl_Fast::switch_sequential(Event& event)
{
	// Add to empty list i.e. switch without erasing the datablock.

	if (data_list[sequential_logicalblock_address] != -1)
		Block_manager::instance()->invalidate(
		    Address(data_list[sequential_logicalblock_address], BLOCK), DATA);

	data_list[sequential_logicalblock_address] =
	    sequential_address.get_linear_address();
...
void
FtlImpl_Fast::merge_sequential(Event& event)
{
	...
	for (uint i = 0; i < BLOCK_SIZE; i++) {
		...
		Address seq =
		    Address(sequential_address.get_linear_address() + i, PAGE);
		if (get_state(seq) == VALID)
			readAddress = seq;
		else if (data_list[sequential_logicalblock_address] != -1 &&
		         get_state(Address(
		                       data_list[sequential_logicalblock_address] + i, PAGE)) ==
		         VALID)
			readAddress.set_linear_address(
			    data_list[sequential_logicalblock_address] + i, PAGE);
		else
			continue; // Empty page

		Event readEvent =
		    Event(READ, event.get_logical_address(), 1, event.get_start_time());
		...
		Event writeEvent =
		    Event(WRITE, event.get_logical_address(), 1,
		          event.get_start_time() + readEvent.get_time_taken());
		writeEvent.set_payload((char*)page_data +
		                       readAddress.get_linear_address() * PAGE_SIZE);
		writeEvent.set_address(
		    Address(newDataBlock.get_linear_address() + i, PAGE));
...
bool
FtlImpl_Fast::random_merge(LogPageBlock* logBlock, Event& event)
{
	...
	for (CI m = mergeBlocks.begin(); m != mergeBlocks.end(); ++m) {
		...
		for (int logblockNr = FAST_LOG_PAGE_LIMIT; logblockNr > 0;
		        logblockNr--) {
			LogPageBlock* lpb = log_pages;
			for (int i = 0; i < logblockNr - 1; i++)
				lpb = lpb->next;

			// Go though the pages and see if any falls into the same category
			// as the current logical block
			for (int i = lpb->numPages - 1; i > 0; i--) {
				...
				// See if there is a conflict, if there isn't. Read and rewrite
				// the page.
				long currentLBA = (lpb->aPages[i] >> addressShift);
				if (victimLBA == currentLBA) {
					Address writeAddress =
					    Address(mergeAddress.get_linear_address() +
					            (lpb->aPages[i] % BLOCK_SIZE),
					            PAGE);
					if (pinned[lpb->aPages[i] % BLOCK_SIZE]) {
						...
						Event readEvent =
						    Event(READ, event.get_logical_address(), 1,
						          event.get_start_time());
						Address readAddress =
						    Address(lpb->address.get_linear_address() + i, PAGE);
						readEvent.set_address(readAddress);
						...
						Event writeEvent = Event(
						                       WRITE, event.get_logical_address(), 1,
						                       event.get_start_time() + readEvent.get_time_taken());
						writeEvent.set_payload(
						    (char*)page_data +
						    readAddress.get_linear_address() * PAGE_SIZE);
		...
		for (uint i = 0; i < BLOCK_SIZE; i++) {
			...
			Address writeAddress =
			    Address(mergeAddress.get_linear_address() + i, PAGE);
			if (get_state(writeAddress) == EMPTY && pinned[i] == false) {
				...
				if (get_state(readAddress) == VALID) {
					Event readEvent = Event(READ, event.get_logical_address(),
					                        1, event.get_start_time());
					readEvent.set_address(readAddress);
					if (controller.issue(readEvent) == FAILURE) {
						printf("failed\n");
						return false;
					...
					Event writeEvent = Event(WRITE, event.get_logical_address(),
					                         1, event.get_start_time() +
					                         readEvent.get_time_taken());
					writeEvent.set_payload((char*)page_data +
					                       readAddress.get_linear_address() *
					                       PAGE_SIZE);
					writeEvent.set_address(writeAddress);
					if (controller.issue(writeEvent) == FAILURE) {
						printf("failed\n");
		...
		// Invalidate inactive pages
		Block_manager::instance()->invalidate(
		    Address(data_list[victimLBA], BLOCK), DATA);
...
bool
FtlImpl_Fast::write_to_log_block(Event& event, long logicalBlockAddress)
{
	uint lbnOffset = event.get_logical_address() % BLOCK_SIZE;
	if (lbnOffset == 0) { /* Case 1 in Figure 5 */
		if (sequential_offset == BLOCK_SIZE) {
			 ...
			 * After switch, the data block is erased and returned to the
			 * free-block list
			 */
			 ...
			 * after merge, the two blocks are erased and returned to the
			 * free-block list
			 */
		...
		if (sequential_logicalblock_address ==
		        logicalBlockAddress) // If the current owner for the SW log block is
			// the same with lbn
		{
			// last_lsn = getLastLsnFromSMT(lbn) Sector mapping table

			if (lbnOffset ==
			        sequential_offset) { // lsn is equivalent with (last_lsn+1)
				// Append data to the SW log block
				...
				// Merge the SW log block with its corresponding data block
				// Get a block from the free-block list and use it as a SW log
				// block
				merge_sequential(event);
			...
			if (log_page_next ==
			        FAST_LOG_PAGE_LIMIT * BLOCK_SIZE) // There are no room in the RW
				// log lock to write data
			{
				/*
				...
				LogPageBlock* newLPB = new LogPageBlock();
				newLPB->address =
				    Block_manager::instance()->get_free_block(LOG, event);
			...
			victim->aPages[log_page_next % BLOCK_SIZE] =
			    event.get_logical_address();
			victim->numPages++;
...
void
FtlImpl_Fast::update_map_block(Event& event)
{
	Event writeEvent =
	    Event(WRITE, event.get_logical_address(), 1, event.get_start_time());
...
void
FtlImpl_Fast::print_ftl_statistics()
{
