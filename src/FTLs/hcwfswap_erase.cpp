...
FtlImpl_HCSwapWF_Erase::FtlImpl_HCSwapWF_Erase(Controller& controller, HotColdID* hcID, double swap_probability, uint d)
    : FtlImpl_HCSwapWF(controller, hcID, swap_probability, d)
{
}

FtlImpl_HCSwapWF_Erase::~FtlImpl_HCSwapWF_Erase() = default;

Address FtlImpl_HCSwapWF_Erase::choose_swap_block(const Address& victim, bool replacingHWF)
{
	...
	if (dSwap == 0) {
        greedy_block_same_plane_min_erase(controller, swapBlock, ignorePred);
	} else {
        d_choices_block_same_plane_min_erase(controller, dSwap, swapBlock, ignorePred);
	}
	return swapBlock;
}
