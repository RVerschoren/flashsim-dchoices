...
void
SSD_Write(unsigned long long address, int size, void* /*buf*/)
{
	gettimeofday(&ssd_request_time, NULL);

	double time =
	    ((ssd_request_time.tv_sec - ssd_boot_time.tv_sec) * 1000 +
	     (ssd_request_time.tv_usec - ssd_boot_time.tv_usec) / 1000.0) +
	    0.5;
...
void
SSD_Read(unsigned long long address, int size, void* /*buf*/)
{
	gettimeofday(&ssd_request_time, NULL);
