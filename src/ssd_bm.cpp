...
void
Block_manager::cost_insert(Block* b)
{
	active_cost.push_back(b);
}

void
Block_manager::instance_initialize(FtlParent* ftl)
{
	Block_manager::inst = new Block_manager(ftl);
}

Block_manager*
Block_manager::instance()
{
 ...
 */
void
Block_manager::get_page_block(Address& address, Event& event)
{
	// We need separate queues for each plane? communication channel?
	// communication channel is at the per die level at the moment. i.e. each
	// LUN is a die.
...
Address
Block_manager::get_free_block(Event& event)
{
 ...
 */
void
Block_manager::promote_block(block_type to_type)
{
	if (to_type == DATA) {
		data_active++;
		log_active--;
 ...
 */
bool
Block_manager::is_log_full()
{
	return log_active == max_log_blocks;
}

void
Block_manager::print_statistics()
{
	printf("-----------------\n");
...
void
Block_manager::invalidate(Address address, block_type type)
{
	invalid_list.push_back(ftl->get_block(address));
 ...
 */
void
Block_manager::insert_events(Event& event)
{
	...
	// First step and least expensive is to go though invalid list. (Only used
	// by FAST)
...
Address
Block_manager::get_free_block(block_type type, Event& event)
{
	Address address;
	get_page_block(address, event);
	switch (type) {
	case DATA:
		ftl->controller.get_block(address)->set_block_type(DATA);
		data_active++;
		break;
	case LOG:
		...
		ftl->controller.get_block(address)->set_block_type(LOG);
		log_active++;
		break;
	default:
		break;
...
void
Block_manager::print_cost_status()
{
                                    ...
                                    block_type btype)
{
	...
	free_list.push_back(ftl->get_block(address));
...
int
Block_manager::get_num_free_blocks()
{
	if (simpleCurrentFree < max_blocks * BLOCK_SIZE)
...
void
Block_manager::update_block(Block* b)
{
