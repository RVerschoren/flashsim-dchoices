...
Bus::Bus(uint num_channels, double ctrl_delay, double data_delay,
         uint table_size, uint max_connections)
	  ...
	  channels((Channel*)malloc(num_channels * sizeof(Channel)))
{
	assert(table_size > 0);
	if (ctrl_delay < 0.0) {
		fprintf(stderr, "Bus warning: %s: constructor received negative "
		        "control delay value\n\tsetting control delay to 0.0\n",
		        __func__);
		ctrl_delay = 0.0;
	}
	if (data_delay < 0.0) {
		fprintf(stderr, "Bus warning: %s: constructor received negative data "
	...
	if (channels == NULL) {
		fprintf(stderr,
		        "Bus error: %s: constructor unable to allocate Channels\n",
		        __func__);
		exit(MEM_ERR);
	}
	for (i = 0; i < num_channels; i++)
		(void)new (&channels[i])
		Channel(ctrl_delay, data_delay, table_size, max_connections);
