...
Channel::Channel(double ctrl_delay, double data_delay, uint /*table_size*/,
...
{
	if (ctrl_delay < 0.0) {
		fprintf(stderr, "Bus channel warning: %s: constructor received "
		        "negative control delay value\n\tsetting control delay "
		        "to 0.0\n",
		        __func__);
		ctrl_delay = 0.0;
	}
	if (data_delay < 0.0) {
		fprintf(stderr, "Bus channel warning: %s: constructor received "
		        "negative data delay value\n\tsetting data delay to "
		        "0.0\n",
		        __func__);
		data_delay = 0.0;
	}
...
{
	if (num_connected > 0)
		fprintf(stderr, "Bus channel warning: %s: %d connected devices when "
		        "bus channel terminated\n",
...
Channel::connect(void) {
	...
	{
		fprintf(stderr, "Bus channel error: %s: device attempting to connect "
		        "to channel when %d max devices already connected\n",
 ...
 * 	only components that receive a single channel should connect */
enum status
Channel::disconnect(void) {
	if (num_connected > 0)
	{
		num_connected--;
		return SUCCESS;
	}
	fprintf(stderr, "Bus channel error: %s: device attempting to disconnect "
	        "from bus channel when no devices connected\n",
	        __func__);
	return FAILURE;
}
 ...
 */
enum status
Channel::lock(double start_time, double duration, Event& event) {
	...
	else
	{
		...
		if ((*it).lock_time > start_time &&
		        (*it).lock_time - start_time >= duration)
			sched_time = start_time;
			...
			for (; it < timings.end(); it++) {
				if (it + 1 != timings.end()) {
					...
					if ((*it).unlock_time >= start_time &&
					        (*(it + 1)).lock_time - (*it).unlock_time >= duration) {
...
void
Channel::unlock(double start_time)
{
...
bool
Channel::timings_sorter(lock_times const& lhs, lock_times const& rhs)
{
	return lhs.lock_time < rhs.lock_time;
}

double
Channel::ready_time(void)
{
	return ready_at;
}
