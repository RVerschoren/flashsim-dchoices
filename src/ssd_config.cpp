...
/*
 * Implementation to use (0 -> Page, 1 -> BAST, 2 -> FAST, 3 -> DFTL, 4 ->
 * BiModal
 */
...
/*
 * Hot/Cold data and false positives/negatives
 */
double HOT_FRACTION = 0.20;
double HOT_REQUEST_RATIO = 0.80;
double HOT_SPARE_FACTOR_FRACTION = 0.50;
double HCID_FALSE_POS_RATIO = 0.0;
double HCID_FALSE_NEG_RATIO = 0.0;
double HCSWAPWF_PROBABILITY = 1.0;

/*
 * Threshold for PE cycle count.
 * This number of blocks must have a certain PE cycle count to advance to the next maximum number of PE cycles.
 */
ulong MAX_PE = 5000;
ulong MAXPE_THRESHOLD = 1;
...
/*
 * Maximum difference in erase counters, for use in HCWFWDIFF
 */
uint MAX_ERASE_DIFF = 0;

/*
 * Wear leveler to use (0 -> None, 1 -> Ban,, 2 -> Ban_Prob, 3 -> DChoicesMax)
 */
uint WEAR_LEVELER = 0;
...
void load_entry(char* name, double value, uint line_number)
{
    ...
    else if (!strcmp(name, "SPARE_FACTOR"))
        SPARE_FACTOR = value;
    else if (!strcmp(name, "HOT_FRACTION"))
        HOT_FRACTION = value;
    else if (!strcmp(name, "HOT_REQUEST_RATIO"))
        HOT_REQUEST_RATIO = value;
    ...
    else if (!strcmp(name, "GC_ALGORITHM"))
        GC_ALGORITHM = value;
    else if (!strcmp(name, "DCHOICES_D"))
        DCHOICES_D = (uint)value;
    else if (!strcmp(name, "WEAR_LEVELER"))
...
void print_config(FILE* stream)
{
    ...
    fprintf(stream, "SPARE_FACTOR: %4lf\n", SPARE_FACTOR);
    fprintf(stream, "FTL_IMPLEMENTATION: %i\n", FTL_IMPLEMENTATION);
    fprintf(stream, "GC_ALGORITHM: %u\n", GC_ALGORITHM);
    fprintf(stream, "DCHOICES_D: %u\n", DCHOICES_D);
    fprintf(stream, "WEAR_LEVELER: %u\n", WEAR_LEVELER);
    fprintf(stream, "WLVL_BAN_TAU: %lu\n", WLVL_BAN_TAU);
    fprintf(stream, "WLVL_BAN_P: %4lf\n", WLVL_ACTIVATION_PROBABILITY);
