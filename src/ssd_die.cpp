...
Die::Die(const Package& parent, Channel& channel, uint die_size, long physical_address)
    : size(die_size),
      ...
      data((Plane*)malloc(size * sizeof(Plane))), parent(parent), channel(channel),
...
enum status Die::merge(Event& event)
{
	assert(data != NULL);
    assert(event.get_address().plane < size && event.get_address().valid > DIE &&
           event.get_merge_address().plane < size && event.get_merge_address().valid > DIE);
...
const Package& Die::get_parent(void) const { return parent; }
...
ssd::Page* Die::get_page_pointer(const Address& addr)
{
	assert(addr.valid >= PLANE);
	return data[addr.plane].get_page_pointer(addr);
}

ssd::Block* Die::get_block_pointer(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_block_pointer(address);
}

ssd::Plane* Die::get_plane_pointer(const Address& address)
{
	assert(address.valid >= PLANE);
	return &data[address.plane];
}

uint Die::get_block_pages_valid(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_block_pages_valid(address);
}

uint Die::get_block_pages_invalid(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_block_pages_invalid(address);
}

uint Die::get_block_pages_erased(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_block_pages_erased(address);
}

void Die::set_block_hotness(const Address& address, const bool hotness)
{
	assert(address.valid >= PLANE);
...
bool Die::get_block_hotness(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_block_hotness(address);
}

enum status Die::get_next_page(Address& address) const
{
	assert(address.valid >= PLANE);
...
ulong Die::get_block_erase_count(const Address& address) const
{
    assert(address.valid >= PLANE);
    return data[address.plane].get_block_erase_count(address);
}
