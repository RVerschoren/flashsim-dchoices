	  ...
	  last_erase_time(0.0)
{
	...
	if (channel.connect() == FAILURE)
		fprintf(stderr,
		        "Die error: %s: constructor unable to connect to Bus Channel\n",
		        __func__);
	...
	if (data == NULL) {
		fprintf(stderr,
		        "Die error: %s: constructor unable to allocate Plane data\n",
		        __func__);
		exit(MEM_ERR);
	}
	...
	for (i = 0; i < size; i++)
		(void)new (&data[i])
		Plane(*this, PLANE_SIZE, PLANE_REG_READ_DELAY, PLANE_REG_WRITE_DELAY,
		      physical_address + (PLANE_SIZE * BLOCK_SIZE * i));
...
enum status
Die::merge(Event& event) {
	assert(data != NULL);
	assert(event.get_address().plane < size &&
	       event.get_address().valid > DIE &&
	       event.get_merge_address().plane < size &&
	       event.get_merge_address().valid > DIE);
	if (event.get_address().plane != event.get_merge_address().plane)
		return _merge(event);
	else
		return data[event.get_address().plane]._merge(event);
}

/* TODO: update stub as per Die::merge() comment above
 * to support Die-level merge operations */
enum status
Die::_merge(Event& event) {
	assert(data != NULL);
	assert(event.get_address().plane < size &&
	       event.get_address().valid > DIE &&
	       event.get_merge_address().plane < size &&
...
double
Die::get_last_erase_time(const Address& address) const
{
	assert(data != NULL);
 ...
 * else return local value */
ssd::ulong
Die::get_erases_remaining(const Address& address) const
{
	assert(data != NULL);
	if (address.valid > DIE && address.plane < size)
...
void
Die::update_wear_stats(const Address& address)
{
	assert(data != NULL);
	uint i;
	uint max_index = 0;
...
void
Die::get_least_worn(Address& address) const
{
...
ssd::uint
Die::get_num_free(const Address& address) const
{
...
ssd::uint
Die::get_num_valid(const Address& address) const
{
	assert(address.valid >= PLANE);
...
ssd::uint
Die::get_num_invalid(const Address& address) const
{
	assert(address.valid >= PLANE);
...
ssd::Page*
Die::get_page_pointer(const Address& addr)
{
	assert(addr.valid >= PLANE);
	return data[addr.plane].get_page_pointer(addr);
}

ssd::Block*
Die::get_block_pointer(const Address& address) const
{
	assert(address.valid >= PLANE);
...
ssd::Plane*
Die::get_plane_pointer(const Address& address)
{
	assert(address.valid >= PLANE);
	return &data[address.plane];
}

uint
Die::get_pages_valid(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_pages_valid(address);
}

uint
Die::get_pages_invalid(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_pages_invalid(address);
}

uint
Die::get_pages_erased(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_pages_erased(address);
}

void
Die::set_block_hotness(const Address& address, const bool hotness)
{
	assert(address.valid >= PLANE);
	data[address.plane].set_block_hotness(address, hotness);
	assert(get_block_hotness(address) == hotness);
}

bool
Die::get_block_hotness(const Address& address) const
{
	assert(address.valid >= PLANE);
	return data[address.plane].get_block_hotness(address);
}

enum status
Die::get_next_page(Address& address) const {
	assert(address.valid >= PLANE);
	data[address.plane].get_next_page(address);
	return SUCCESS;
}
