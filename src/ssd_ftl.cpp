...
enum status
Ftl::read(Event& event) {
	return SUCCESS;
}

enum status
Ftl::write(Event& event) {
	return SUCCESS;
}

enum status
Ftl::erase(Event& event) {
	return SUCCESS;
}

enum status
Ftl::merge(Event& event) {
	return SUCCESS;
}

void
Ftl::garbage_collect(Event& event)
{
	return;
}

ssd::ulong
Ftl::get_erases_remaining(const Address& address) const
{
	return controller.get_erases_remaining(address);
}

void
Ftl::get_least_worn(Address& address) const
{
...
enum page_state
Ftl::get_state(const Address& address) const {
	return controller.get_state(address);
}

ssd::Page*
Ftl::get_page(const Address& addr) const
{
	return controller.get_page(addr);
}

ssd::Block*
Ftl::get_block(const Address& addr) const
{
	return controller.get_block(addr);
}

ssd::Plane*
Ftl::get_plane(const Address& addr) const
{
