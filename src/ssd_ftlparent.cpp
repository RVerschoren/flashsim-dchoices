...
FtlParent::FtlParent(Controller& controller) : controller(controller), garbage(nullptr)
{
	...
	case GC_DCHOICES:
		...
		break;
    case GC_DCH_HOTCOLD:
        garbage = new GCImpl_DCh_HotCold(this);
        break;
    case GC_DCHOICES_ERASETIE:
        garbage = new GCImpl_DCh_TieErasures(this);
        break;
    case GC_DCHOICES_ERASE:
        garbage = new GCImpl_DCh_Erase(this);
        break;
    case GC_DCHOICES_ERASE_VALIDTIE:
        garbage = new GCImpl_DCh_Erase_TieValid(this);
        break;
	case GC_COSTBENEFIT:
	...
	case WL_HOTCOLDSWAP:
		...
		break;
    case WL_HOTCOLDGCSWAP:
		wlvl = new WLvlImpl_HotColdGCSwap(this);
        break;
    case WL_HOTCOLDGCSWAP_ERASE:
        wlvl = new WLvlImpl_HotColdGCSwap_Erase(this);
        break;
	default:
		wlvl = new WLvlImpl_Ban(this);
	}
...
void FtlParent::initialize(const ulong /*numUniqueLPN*/)
{
	assert(false);
	return;
}

void FtlParent::initialize(const std::set<ulong>& /*uniqueLPNs*/)
{
	assert(false);
	return;
}
...
ulong FtlParent::get_block_erase_count(const Address& address) const
{
    return controller.get_block_erase_count(address);
}
...
enum page_state FtlParent::get_state(const Address& address) const { return controller.get_state(address); }
...
Block* FtlParent::get_block(const Address& address) const { return controller.get_block(address); }
...
void FtlParent::print_ftl_statistics() { return; }
...
enum status FtlParent::swap(Event& evt, const Address& block1, const Address& block2)
{
	std::function<void(ulong, const Address&)> modifier = [this](ulong lpn, const Address& addr) {
		this->modifyFTL(lpn, addr);
	};
	return Block::_swap(this, evt, block1, block2, modifier);
}

void FtlParent::modifyFTL(const ulong /*lpn*/, const Address& /*newAddress*/) {}

void FtlParent::set_block_hotness(const Address& address, const bool hotness)
{
	assert(address.valid > NONE);
...
bool FtlParent::get_block_hotness(const Address& address) const
{
	assert(address.valid > NONE);
	return controller.get_block_hotness(address);
}

enum status FtlParent::get_next_page(Address& address) const
{
	assert(address.valid > NONE);
