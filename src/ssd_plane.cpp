      ...
      free_blocks(size)
{
    ...
    if (reg_read_delay < 0.0) {
        fprintf(stderr,
                "Plane error: %s: constructor received negative "
    ...
    for (i = 0; i < size; i++) {
        (void)new (&data[i])
            Block(*this, BLOCK_SIZE, BLOCK_ERASES, BLOCK_ERASE_DELAY, physical_address + (i * BLOCK_SIZE));
    }
...
ssd::uint Plane::get_size(void) const { return size; }

const Die& Plane::get_parent(void) const { return parent; }
...
Block* Plane::get_block_pointer(const Address& address) const
{
    assert(address.valid >= BLOCK);
    return static_cast<Block*>(&(data[address.block])); // data[address.block].get_pointer();
}

ssd::Page* Plane::get_page_pointer(const Address& addr)
{
    assert(addr.valid >= BLOCK);
    return data[addr.block].get_page_pointer(addr);
}

uint Plane::get_block_pages_valid(const Address& address) const
{
    assert(address.valid >= BLOCK);
    return data[address.block].get_pages_valid();
}

uint Plane::get_block_pages_invalid(const Address& address) const
{
    assert(address.valid >= BLOCK);
    return data[address.block].get_pages_invalid();
}

uint Plane::get_block_pages_erased(const Address& address) const
{
    assert(address.valid >= BLOCK);
...
void Plane::set_block_hotness(const Address& address, const bool hotness)
{
    assert(address.valid >= BLOCK);
...
bool Plane::get_block_hotness(const Address& address) const
{
    assert(address.valid >= BLOCK);
    return data[address.block].get_block_hotness();
}

ulong Plane::get_block_erase_count(const Address& address) const
{
    assert(address.valid >= BLOCK);
    return data[address.block].get_erase_count();
}

enum status Plane::get_next_page(Address& address) const
{
    assert(address.valid >= BLOCK);
