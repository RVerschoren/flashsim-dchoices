...
Plane::Plane(const Die& parent, uint plane_size, double reg_read_delay,
             double reg_write_delay, long physical_address)
	: size(plane_size)
	,
	  ...
	  free_blocks(size)
{
	...
	if (reg_read_delay < 0.0) {
		fprintf(stderr, "Plane error: %s: constructor received negative "
		        "register read delay value\n\tsetting register read "
		        "delay to 0.0\n",
		        __func__);
		reg_read_delay = 0.0;
	} else
	 ...
	 * plane does not know about higher-level hardware organization, so we
	 * cannot
	...
	if (data == NULL) {
		fprintf(stderr,
		        "Plane error: %s: constructor unable to allocate Block data\n",
		        __func__);
		exit(MEM_ERR);
	}
	...
	for (i = 0; i < size; i++) {
		(void)new (&data[i])
		Block(*this, BLOCK_SIZE, BLOCK_ERASES, BLOCK_ERASE_DELAY,
...
enum status
Plane::read(Event& event) {
	assert(event.get_address().block < size &&
	       event.get_address().valid > PLANE);
	return data[event.get_address().block].read(event);
}

enum status
Plane::write(Event& event) {
	assert(event.get_address().block < size &&
	       event.get_address().valid > PLANE && next_page.valid >= BLOCK);
...
Plane::_merge(Event& event) {
	...
	if (merge_count > merge_avail)
	{
		fprintf(
		    stderr,
		    "Plane error: %s: Not enough space to merge block %d into block %d\n",
	...
	for (i = 0; num_merged < merge_count && read.page < block_size;
	        read.page++)
	{
		...
		if (data[read.block].get_state(read.page) == VALID) {
			...
			if (data[read.block].read(read_event) == 0) {
				fprintf(
				    stderr,
				    "Plane error: %s: Read for merge block %d into %d failed\n",
			...
			for (; write.page < merge_block_size; write.page++) {
				...
				if (data[write.block].get_state(write.page) == EMPTY) {
					...
					if (data[merge_address.block].write(write_event) == 0) {
						fprintf(stderr, "Plane error: %s: Write for merge "
						        "block %d into %d failed\n",
						        __func__, address.block, merge_address.block);
						i++;
					}
...
ssd::uint
Plane::get_size(void) const
{
	return size;
}

const Die&
Plane::get_parent(void) const
{
	return parent;
}
...
double
Plane::get_last_erase_time(const Address& address) const
{
	assert(data != NULL);
 ...
 * else return local value */
ssd::ulong
Plane::get_erases_remaining(const Address& address) const
{
	assert(data != NULL);
	if (address.valid > PLANE && address.block < size)
...
void
Plane::get_least_worn(Address& address) const
{
...
void
Plane::get_free_page(Address& address) const
{
	...
	address.set_linear_address(address.get_linear_address() + address.page -
	                           (address.get_linear_address() % BLOCK_SIZE));
#endif
	return;
}
...
ssd::uint
Plane::get_num_free(const Address& address) const
{
...
ssd::uint
Plane::get_num_valid(const Address& address) const
{
	assert(address.valid >= PLANE);
...
ssd::uint
Plane::get_num_invalid(const Address& address) const
{
	assert(address.valid >= PLANE);
...
Block*
Plane::get_block_pointer(const Address& address) const
{
	assert(address.valid >= BLOCK);
	return static_cast<Block*>(
	           &(data[address.block])); // data[address.block].get_pointer();
}

ssd::Page*
Plane::get_page_pointer(const Address& addr)
{
	assert(addr.valid >= BLOCK);
	return data[addr.block].get_page_pointer(addr);
}

uint
Plane::get_pages_valid(const Address& address) const
{
	assert(address.valid >= BLOCK);
	return data[address.block].get_pages_valid();
}
...
uint
Plane::get_pages_invalid(const Address& address) const
{
	assert(address.valid >= BLOCK);
	return data[address.block].get_pages_invalid();
}
...
uint
Plane::get_pages_erased(const Address& address) const
{
	assert(address.valid >= BLOCK);
	const Block& block = data[address.block];
	return BLOCK_SIZE - block.get_pages_valid() - block.get_pages_invalid();
}

void
Plane::set_block_hotness(const Address& address, const bool hotness)
{
	assert(address.valid >= BLOCK);
	data[address.block].set_block_hotness(hotness);
	assert(get_block_hotness(address) == hotness);
}

bool
Plane::get_block_hotness(const Address& address) const
{
	assert(address.valid >= BLOCK);
	return data[address.block].get_block_hotness();
}

enum status
Plane::get_next_page(Address& address) const {
	assert(address.valid >= BLOCK);
	data[address.block].get_next_page(address);
	return SUCCESS;
}
