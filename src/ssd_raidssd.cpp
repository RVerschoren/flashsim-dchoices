 ...
 *
 * The Raid SSD is responsible for raiding multiple SSDs together using
...
double
RaidSsd::event_arrive(enum event_type type, ulong logical_address, uint size,
                      double start_time)
{
	...
	if (PARALLELISM_MODE == 1) { // Striping
		...
		for (uint i = 0; i < RAID_NUMBER_OF_PHYSICAL_SSDS; i++) {
			if (buffer == NULL) {
				...
				timings[i] = Ssds[i].event_arrive(type, logical_address, size,
				                                  start_time, NULL);
			}

			else
				timings[i] =
				    Ssds[i].event_arrive(type, logical_address, size, start_time,
				                         (char*)buffer + (i * PAGE_SIZE));
		}
		...
		return Ssds[logical_address % RAID_NUMBER_OF_PHYSICAL_SSDS]
		       .event_arrive(type, logical_address, size, start_time, (char*)buffer);
	}
