...
{
	if (read_delay <= 0) {
		fprintf(stderr, "RAM: %s: constructor received negative read delay "
		        "value\n\tsetting read delay to 0.0\n",
		        __func__);
		read_delay = 0.0;
	}
	if (write_delay <= 0) {
		fprintf(stderr, "RAM: %s: constructor received negative write delay "
