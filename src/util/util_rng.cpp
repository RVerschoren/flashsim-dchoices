...
RandNrGen::RandNrGen()
{
	uint32_t seedVal = static_cast<uint32_t>(time(NULL));
	rng.seed(seedVal);
}
...
unsigned long
RandNrGen::get(unsigned long S)
{
	return static_cast<unsigned long>(std::floor(S * dist(rng)));
}

unsigned int
RandNrGen::get(unsigned int S)
{
	return static_cast<unsigned int>(std::floor(S * dist(rng)));
}
