...
main(int argc, char** argv)
{
	...
	fprintf(logFile,
	        "NumIOReads;ReadIOTime;NumIOWrites;WriteIOTime;NumIOTotal;IOTime;");
	...
	//	{
	//
	//		write_time = ssd.event_arrive(WRITE, i, 1,
	//((start_time+arrive_time)*timeMultiplier));
	...
	/// num_writes,
	/// write_time, num_reads+num_writes, read_time+write_time);
	//
	//	}
	...
	//
	//		write_time = ssd.event_arrive(WRITE, r, 1,
	//((start_time+arrive_time)*timeMultiplier));
	//		avgs.push_back(write_time);
	//		num_writes++;
	//
	...
	for (int i = 0; i < preIO * 1.3; i++) {
		...
		write_time = ssd.event_arrive(
		                 WRITE, r, 1, ((start_time + arrive_time) * timeMultiplier));
		avgs.push_back(write_time);
		num_writes++;
	...
	for (int i = preIO / 2 - seqSize / 2; i < preIO / 2 + seqSize; i++) {
		write_time = ssd.event_arrive(
		                 TRIM, i, 1, ((start_time + arrive_time) * timeMultiplier));
		avgs.push_back(write_time);
		num_writes++;
