...
ssd::Block* Die::get_block_pointer(const Address &address)
{
    assert(address.valid >= PLANE);
...
uint Die::get_pages_valid(const Address &address) const
{
    assert(address.valid >= PLANE);
    return data[address.plane].get_pages_valid(address);
}

uint Die::get_pages_invalid(const Address &address) const
{
    assert(address.valid >= PLANE);
    return data[address.plane].get_pages_invalid(address);
}

uint Die::get_pages_erased(const Address &address) const
{
    assert(address.valid >= PLANE);
