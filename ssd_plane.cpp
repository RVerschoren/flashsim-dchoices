...
uint Plane::get_pages_valid(const Address &address) const
{
    assert(address.valid >= BLOCK);
    return data[address.block].get_pages_valid();
}

uint Plane::get_pages_invalid(const Address &address) const
{
    assert(address.valid >= BLOCK);
    return data[address.block].get_pages_invalid();
}

uint Plane::get_pages_erased(const Address &address) const
{
    assert(address.valid >= BLOCK);
