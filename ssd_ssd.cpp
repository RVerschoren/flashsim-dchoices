...
double Ssd::event_arrive(enum event_type type, ulong logical_address, uint size, double start_time, void *buffer)
{
    ...
    ///@TODO THESE ASSUMPTIONS DO NOT HOLD FOR OUR TRACES
...
ssd::Block* Ssd::get_block_pointer(const Address &address)
{
    assert(address.valid >= PACKAGE);
    assert(data != NULL);
...
void Ssd::write_statistics_csv(std::string fileName, uint runID, std::string traceID)
{
